...
describe('built platform-project-governance package entry', () => {
  it('loads the declared root entry', () => {
    const result = importFromPackage('@aurora/platform-project-governance');
    expect(result.status).toBe(0);
    expect(result.stderr).toBe('');
    expect(result.stdout).toContain('PLATFORM_PROJECT_GOVERNANCE_PACKAGE');
    expect(result.stdout).toContain('PLATFORM_PROJECT_GOVERNANCE_VERSION');
  });

  it('rejects private and unexported paths', () => {
    for (const specifier of [
      '@aurora/platform-project-governance/src/index.js',
      '@aurora/platform-project-governance/internal/repositories.js',
      '@aurora/platform-project-governance/repositories',
    ]) {
  ...
  it('declares the private data-layer package manifest', async () => {
    ...
    expect(manifest).toMatchObject({
      name: '@aurora/platform-project-governance',
      private: true,
      type: 'module',
      aurora: { layer: 'data' },
