...
const stageLabel = computed(() => {
  switch (data.value?.stage) {
    case 'near-limit': return '接近限额';
    case 'degraded': return '已降级';
    case 'hard-limit': return '已达硬限';
    default: return '正常';
  }
});
...
<template>
  <section class="au-surface mon-usage" data-testid="usage-view">
    <AppPageHeader title="资源用量" description="查看当前组织统计周期内的权威接收、处理与限额证据。" />
    ...
    <template v-else-if="data !== null">
      <AppSection title="当前周期" :description="`${data.periodStart} — ${data.periodEnd}`" :tone="data.stage === 'normal' ? 'success' : 'warning'">
        <AppStatusBadge :tone="data.stage === 'normal' ? 'success' : 'warning'">{{ stageLabel }}</AppStatusBadge>
        <p v-if="data.note !== undefined" class="mon-note">{{ data.note }}</p>
      </AppSection>
      <AppSection title="用量证据" description="接收、处理与限额分别展示，不以图表或预测替代服务端事实。">
        <dl class="mon-usage-grid">
          <div><dt>已接收事件</dt><dd data-testid="usage-accepted">{{ data.acceptedEvents }}</dd></div>
          <div><dt>已处理证据</dt><dd data-testid="usage-processed">{{ data.processedEvents }}</dd></div>
          <div><dt>周期限额</dt><dd>{{ data.quotaAcceptedEvents }}</dd></div>
          <div><dt>限额使用率</dt><dd>{{ (data.ratio * 100).toFixed(2) }}%</dd></div>
        </dl>
        <AppTechnicalDetails summary="技术详情" data-testid="usage-technical-details">组织 ID: {{ data.organizationId }}</AppTechnicalDetails>
      </AppSection>
    </template>
  </section>
</template>
...
<style scoped>
.mon-usage { display: flex; flex-direction: column; gap: var(--space-5); }
.mon-note { margin: var(--space-2) 0 0; color: var(--color-text-secondary); }
.mon-usage-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: var(--space-3); margin: 0; }
.mon-usage-grid > div { padding: var(--space-3); border: 1px solid var(--color-border-default); border-radius: var(--radius-control); }
.mon-usage-grid dt { color: var(--color-text-secondary); font-size: 12px; }
.mon-usage-grid dd { margin: var(--space-1) 0 0; color: var(--color-text-primary); font-size: 20px; font-weight: 650; }
</style>
