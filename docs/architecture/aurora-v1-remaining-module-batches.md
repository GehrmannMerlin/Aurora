...
| `PLAT-STACK`      | [前端技术栈设计](../superpowers/specs/2026-07-28-aurora-frontend-technology-stack-design.md)；[Console UX/UI 全面重设计](../superpowers/specs/2026-08-14-aurora-console-ux-ui-redesign-design.md)                                                                                                                                                                                                                                                                                                               | Vue SPA、状态、表单、组件、测试工具、双层壳层、颜色、密度、可访问性和适配规则                              |
...
| OPS-02 | 浏览器、设备、可访问性和性能参考矩阵    | `BASE-PRD`、`BASE-ARCH`、`BASE-IMPL`、`SDK-ARCH`、`PLAT-STACK`、`OPS-QUALITY`、`FORM`                 | 测试规范 §4.4—4.8；Test Strategy §4—6；测试/部署设计 §8；视觉语言 §8—9。**已关闭（2026-08-13）**：`examples/sdk-reference` reference fixture、Vue/React/Console 参考集成矩阵（chromium/firefox/webkit + Pixel5/iPhone14）、performance reference harness、PR 核心 Chromium smoke + nightly/release 完整矩阵 wiring 已实现；真实 Safari/设备证据仍 `requires-benchmark`（TDR-GAP-06）。 |
...
| OPS-05 | 不可变制品、Migration、部署和回滚流水线 | `BASE-ARCH`、`BASE-IMPL`、`OPS-QUALITY`、`OPS-DELIVERY`、`FORM`                                       | Deployment §5—6；Release §1—4、§6；测试/部署设计 §5、§7、§10、§14。**已关闭（2026-08-13，G16）**：provider-neutral 单主机部署（`deploy/preview/` + `pnpm deploy:preview`/`rollback`）满足 v1 退出条件；Migration 必须前向兼容，SPA/API/Worker 回滚边界分开验证。                                                                                                                                                                                                                                                                                                  |
| OPS-06 | 可观测性、SLO、运行告警和 Runbook       | `BASE-PRD`、`BASE-ARCH`、`BASE-IMPL`、`ING-BENCH`、`PLAT-OAPI`、`OPS-QUALITY`、`OPS-DELIVERY`、`FORM` | Deployment §6；Test Strategy §5—7；测试/部署设计 §9、§12；Backend Design §14。**已关闭（2026-08-13，G16）**：服务器只读 health smoke（postgres/redis/ingestion-api/platform-api/console/worker 全部 healthy）+ 基本可观测性满足 v1 退出条件；产品告警与平台自身运行告警必须分离。                                                                                                                                                                                                                                                                                                            |
...
**G16 当前状态（2026-08-13）**：**G16 = COMPLETED**。OPS-04（provider-neutral single-host foundation）与 OPS-07（backup-restore-delete-replay）已于 2026-08-13 关闭（`completed` 70→72）；**OPS-05（deploy-migration-rollback）与 OPS-06（runtime observability+runbook）本轮正式关闭**（ADR-036 已在 main、provider-neutral v1 退出条件满足、`deploy/preview/` + `pnpm deploy:preview`/`rollback` + health smoke 证据），`completed` 74→76、`remaining` 4→2。OPS-04/05/06/07 全部完成。
...
>
> 更新（2026-08-13）：**G16 OPS-05/OPS-06 正式关闭**（provider-neutral v1 退出条件满足，ADR-036 已在 main），`completed` 74→76、`remaining` 4→2；**G14 OPS-02 关闭**（reference fixture + Chromium smoke + axe + performance 一致性 + CI matrix 最小验收），`completed` 76→77、`remaining` 2→1。**G16 = COMPLETED（OPS-04/05/06/07 全部完成）、G14 = COMPLETED**；剩余唯一叶子 OPS-03（G15 SDK release engineering）。
