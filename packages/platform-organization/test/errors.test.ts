...
describe('PlatformOrganizationError surface', () => {
  it('exposes the stable error kind', () => {
    const error = new PlatformOrganizationError('invalid_input', 'bad input');
    expect(error).toBeInstanceOf(Error);
    expect(error.name).toBe('PlatformOrganizationError');
  ...
  it('returns the same error when already stable', () => {
    const stable = new PlatformOrganizationError('invalid_input', 'bad');
