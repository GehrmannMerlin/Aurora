    ...
    <template v-else>
      <section class="mon-block" data-testid="alert-instance-status">
        ...
        <AppTechnicalDetails summary="实例技术详情">
          instanceId: {{ state.instance.data.instanceId }} ruleId:
          {{ state.instance.data.ruleId }} state: {{ state.instance.data.state }} directReason:
          {{ state.instance.data.directReason }}
        </AppTechnicalDetails>
      </section>
        ...
        <template v-else>
          <ol v-if="state.transitions.data.length > 0" class="mon-transition-list">
            ...
            >
              ...
              >
              <span class="mon-meta"
                >{{ alertReasonLabel(transition.reason) }} ·
                {{ formatUtc(transition.occurredAt) }}</span
              >
            </li>
          </ol>
