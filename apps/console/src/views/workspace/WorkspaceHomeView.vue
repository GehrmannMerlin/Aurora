  ...
  <section data-testid="workspace-home" class="au-surface">
    ...
    <template v-else>
      ...
      <template v-if="activeOrg !== null && loadError === null">
        <AppSection
          :title="`${activeOrg.name} 的项目`"
          description="仅展示当前组织范围内允许访问的项目。"
        >
          <AppSkeleton
            v-if="loading"
            label="正在加载项目…"
          ...
          <p
            v-if="!loading && projects.length === 0 && !canCreateProject"
            class="au-hint"
            data-testid="create-project-forbidden"
          >
            你当前没有在该组织中创建项目的权限。
          </p>
          <ul v-else class="au-project-list" data-testid="project-list">
            <li
              ...
              class="au-project-item"
              data-testid="project-row"
            >
              <div class="au-project-item__identity">
                <AppLink :to="projectHref(project.projectId)">{{ project.name }}</AppLink>
                <span class="au-project-meta" data-testid="project-framework"
                  >接入类型：{{ frameworkLabel[project.frameworkType] }}</span
                >
              </div>
              <AppStatusBadge
                :tone="project.lifecycle === 'active' ? 'success' : 'neutral'"
              ...
              <AppLink
                :to="projectHref(project.projectId)"
...
.au-project-item:last-child {
  border-bottom: 0;
}
.au-project-item__identity {
  display: grid;
  gap: var(--space-1);
}
.au-project-meta {
