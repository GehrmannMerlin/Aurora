...
> 更新（2026-08-08）：PLT-02 implemented-in-feature-branch（未部署），`apps/console` Vue 3 SPA 壳层 + 生产构建门禁 + Preview 切换文件已落盘；**计数保持 completed 39 / remaining 39，等待 PLT-02 独立验收**。
...
| PLT-02 | Session、导航、RouteTarget 和 Vue SPA 壳层 | `BASE-PRD`、`BASE-ARCH`、`BASE-IMPL`、`PLAT-DOMAINS`、`PLAT-UX`、`PLAT-STACK`、`PLAT-OAPI`、`FORM`                                        | UX/UI §5、§7.1、§12.1—12.9；OpenAPI 设计 §11—15、§17；Frontend Architecture §2—7；视觉语言 §2—10。必须实现真实可达性，不允许仅手输 URL。**已 implemented-in-feature-branch（未部署，2026-08-08）**：`apps/console` Vue 3 SPA 壳层（36 个 RouteTarget 真实可达、Session/Navigation Context、Aurora UI shell、状态页、Playwright 可达性与 axe 门禁、`test:package` 生产构建门禁、Preview 切换文件）；叶子计数保持 completed 39 / remaining 39，待 PLT-02 独立验收。 |
...
| OPS-02 | 浏览器、设备、可访问性和性能参考矩阵    | `BASE-PRD`、`BASE-ARCH`、`BASE-IMPL`、`SDK-ARCH`、`PLAT-STACK`、`OPS-QUALITY`、`FORM`                 | 测试规范 §4.4—4.8；Test Strategy §4—6；测试/部署设计 §8；视觉语言 §8—9。必须有参考应用和可重复测量环境。                                         |
...
>
> 更新（2026-08-08）：**OPS-01 已关闭（completed）**，`completed` 37→38、`remaining` 41→40；**OPS-02 blocked**（reference app/Console/device matrix/performance env 缺失，不伪造关闭），G14 = partially completed；**PLT-01 已关闭（completed）**，`completed` 38→39、`remaining` 40→39（独立验收通过）。**PLT-02 implemented-in-feature-branch（未部署）**：`apps/console` Vue 3 SPA 壳层 + 生产构建门禁 + Preview 切换文件已落盘，**计数保持 completed 39 / remaining 39，待 PLT-02 独立验收**。下表反映更新后的剩余覆盖。
