...
const issuesHref = computed(() =>
  actionTargetHref({ routeId: 'project.issues', pathParams: { organizationId, projectId }, query: {} }),
);
...
<template>
  <section class="au-surface mon-workspace" data-testid="project-overview-view">
    <AppPageHeader title="项目概览" description="从权威接收状态开始核对，再查看各项独立证据。">
      <template #actions>
        <button type="button" class="au-button" :disabled="diagnosis.loading.value" @click="refresh">
          刷新证据
        </button>
      </template>
    </AppPageHeader>
    ...
    <section class="mon-evidence-grid" data-testid="overview-evidence" aria-label="项目证据">
      <AppSection title="问题" description="当前查询窗口中的问题事实。">
        ...
        <template v-else>
          <p class="mon-count-line">
            {{ state.issues.data.totalCountStatus === 'available' ? state.issues.data.totalCount : '—' }}
            <span class="mon-meta">当前窗口内问题总数</span>
          </p>
          <p v-if="state.issues.data.totalCountStatus !== 'available'" class="mon-note">总数不可用。</p>
        ...
        <template v-else>
          <dl v-if="state.requests.data.methods.length > 0" class="mon-inline">
            <div v-for="method in state.requests.data.methods" :key="method.method">
              <dt>{{ method.method }}</dt>
              <dd>{{ method.observedCount }} 次 <span class="mon-meta">失败 {{ method.failureCount }} · 慢 {{ method.slowCount }}</span></dd>
            </div>
          </dl>
          <p v-else class="mon-note">窗口内没有请求指标。</p>
          <p v-if="state.requests.data.dataThrough" class="mon-meta">数据截止（UTC）：{{ formatUtc(state.requests.data.dataThrough) }}<template v-if="state.requests.data.isPartial"> · 部分结果</template></p>
        </template>
      </AppSection>
        ...
        <template v-else>
          <dl v-if="state.performance.data.metrics.length > 0" class="mon-inline">
            <div v-for="metric in state.performance.data.metrics" :key="metric.metricName">
              ...
              <dd>{{ metric.mean }} {{ metricUnit(metric.unit) }} <span class="mon-meta">样本 {{ metric.observedCount }}</span></dd>
            </div>
          </dl>
          <p v-else class="mon-note">窗口内没有性能指标。</p>
          <p v-if="state.performance.data.dataThrough" class="mon-meta">数据截止（UTC）：{{ formatUtc(state.performance.data.dataThrough) }}<template v-if="state.performance.data.isPartial"> · 部分结果</template></p>
        </template>
      </AppSection>
      ...
      <AppSection title="最近数据" description="接收与处理是彼此独立的阶段事实。">
        ...
        <dl v-else class="mon-inline">
          <div><dt>最近接收</dt><dd>{{ state.recent.data.receivedCount }}<span v-if="state.recent.data.latestReceivedAt" class="mon-meta"> · {{ formatUtc(state.recent.data.latestReceivedAt) }}</span></dd></div>
          <div><dt>最近已处理</dt><dd>{{ state.recent.data.processedCount }}<span v-if="state.recent.data.latestProcessedAt" class="mon-meta"> · {{ formatUtc(state.recent.data.latestProcessedAt) }}</span></dd></div>
        </dl>
      </AppSection>
      ...
      <AppSection title="可查询证据" description="只有进入处理存储的证据才能查询。">
        ...
        <dl v-else class="mon-inline">
          <div><dt>错误事件</dt><dd>{{ state.queryable.data.errorOccurrences }}</dd></div>
          <div><dt>请求指标桶</dt><dd>{{ state.queryable.data.requestMetricBuckets }}</dd></div>
          <div><dt>性能指标桶</dt><dd>{{ state.queryable.data.performanceMetricBuckets }}</dd></div>
        </dl>
      </AppSection>
    </section>

    <AppSection v-if="actionTargets.length > 0" title="可执行行动" description="仅显示服务端已授权的目标。" test-id="overview-actions">
      <ul class="mon-actions">
        <li v-for="entry in actionTargets" :key="entry.href"><AppLink :to="entry.href">{{ actionTargetLabel(entry.target.routeId) }}</AppLink></li>
      </ul>
    </AppSection>
  </section>
</template>
...
.mon-authority-line { display: flex; flex-wrap: wrap; align-items: center; gap: var(--space-3); }
.mon-note, .mon-meta { color: var(--color-text-secondary); }
.mon-note { margin: var(--space-2) 0 0; }
.mon-meta { font-size: 12px; }
.mon-count-line { margin: 0 0 var(--space-2); font-size: 24px; font-weight: 650; color: var(--color-text-primary); }
.mon-evidence-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: var(--space-4); }
.mon-inline { display: flex; flex-wrap: wrap; gap: var(--space-4); margin: 0; }
