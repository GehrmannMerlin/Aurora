...
const statusLine = computed(() =>
  summaryView.value.kind === 'available' ? onboardingStatusLine(summaryView.value.data) : null,
);
const authority = computed(() =>
  summaryView.value.kind === 'available' ? summaryDisplay(summaryView.value.data) : null,
);
...
<template>
  <section class="au-surface mon-workspace" data-testid="project-onboarding-view">
    ...
    <AppSection
      title="当前接入证据"
      description="服务端诊断不等同于测试事件已生成问题。"
      :tone="statusLine?.tone ?? 'neutral'"
      test-id="onboarding-status"
    >
      ...
      <template v-else-if="statusLine !== null && authority !== null">
        ...
        <AppTechnicalDetails summary="技术详情"
          >状态键: {{ summaryView.data.status
          }}<template v-if="summaryView.data.primaryCause"
            >\n原因键: {{ summaryView.data.primaryCause }}</template
          ></AppTechnicalDetails
        >
      </template>
    </AppSection>
    ...
    <AppSection
      title="接入步骤"
      description="使用一次性交付密钥完成 SDK 初始化，再以服务端证据确认测试错误已形成 Issue。"
      test-id="onboarding-guide"
    >
      <ol class="mon-onboarding-sequence">
        ...
        <li>
          ...
          <div>
            <h3>发送测试错误</h3>
            <p>
              运行示例或使用下方按钮发送测试错误；只有事件已处理并在 Issues
              中形成对应记录时才显示“接入成功”。
            </p>
            <pre
              class="mon-code"
            ...
            <div class="mon-actions-row">
              <AppButton
                ...
                :disabled="
                  clientKey === null || sendState === 'sending' || sendState === 'accepted'
                "
                @click="sendTestError"
              >
                {{
            ...
            <AppStatusBadge v-if="checkState === 'checking'" tone="warning"
              >正在检查接收、处理与 Issue 聚合（最长 60 秒）…</AppStatusBadge
            >
            <AppStatusBadge v-else-if="checkState === 'timeout'" tone="warning"
              >60 秒内尚未确认接入成功，自动检查已停止。</AppStatusBadge
            >
          </div>
        </li>
      </ol>
    ...
    <section class="mon-evidence-grid" aria-label="接入证据">
      <AppSection
        title="处理阶段"
        description="已接收、处理中、已处理与可查询不能互相替代。"
        test-id="onboarding-stages"
      >
        <SectionNotice v-if="stagesView.kind !== 'available'" :view="stagesView" />
        <dl v-else class="mon-stage-grid">
          <div>
            <dt>已接收（可靠缓冲）</dt>
            <dd>{{ formatCount(stagesView.data.received.count) }}</dd>
            <dd v-if="stagesView.data.received.latestAt" class="mon-stage-meta">
              {{ formatUtc(stagesView.data.received.latestAt) }}
            </dd>
          </div>
          <div>
            <dt>处理中</dt>
            <dd>{{ formatCount(stagesView.data.processing.count) }}</dd>
            <dd v-if="stagesView.data.processing.latestAt" class="mon-stage-meta">
              {{ formatUtc(stagesView.data.processing.latestAt) }}
            </dd>
          </div>
          <div>
            <dt>已处理</dt>
            <dd>{{ formatCount(stagesView.data.processed.count) }}</dd>
            <dd v-if="stagesView.data.processed.latestAt" class="mon-stage-meta">
              {{ formatUtc(stagesView.data.processed.latestAt) }}
            </dd>
          </div>
          <div>
            <dt>死信事件</dt>
            <dd>{{ formatCount(stagesView.data.deadLetter.count) }}</dd>
            <dd v-if="stagesView.data.deadLetter.lastErrorCode" class="mon-stage-meta">
              最近错误见技术详情
            </dd>
          </div>
        </dl>
        <AppTechnicalDetails
          v-if="stagesView.kind === 'available' && stagesView.data.deadLetter.lastErrorCode"
          summary="技术详情"
          >最近错误键: {{ stagesView.data.deadLetter.lastErrorCode }}</AppTechnicalDetails
        >
      </AppSection>
      <AppSection
        title="密钥与可查询证据"
        description="密钥状态与已处理的查询证据分别来自服务端投影。"
      >
        <div class="mon-evidence-stack">
          <div data-testid="onboarding-credential">
            <h3>上报密钥</h3>
            <SectionNotice v-if="credentialView.kind !== 'available'" :view="credentialView" />
          ...
          <div data-testid="onboarding-queryable">
            <h3>可查询证据</h3>
            <SectionNotice v-if="queryableView.kind !== 'available'" :view="queryableView" />
            <dl v-else class="mon-inline">
              <div>
                <dt>错误事件</dt>
...
.mon-onboarding-sequence li {
  display: grid;
  grid-template-columns: 32px minmax(0, 1fr);
  gap: var(--space-3);
}
.mon-onboarding-sequence h3 {
  margin: 0;
  font-size: 15px;
}
.mon-onboarding-sequence p {
  margin: var(--space-2) 0 0;
  color: var(--color-text-secondary);
  max-width: 64ch;
}
