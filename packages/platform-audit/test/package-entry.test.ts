...
describe('built platform-audit package entry', () => {
  it('loads the declared root entry', () => {
    const result = importFromPackage('@aurora/platform-audit');
    expect(result.status).toBe(0);
    expect(result.stderr).toBe('');
    expect(result.stdout).toContain('PLATFORM_AUDIT_PACKAGE');
    expect(result.stdout).toContain('PLATFORM_AUDIT_VERSION');
  });

  it('rejects private and unexported paths', () => {
    for (const specifier of [
      '@aurora/platform-audit/src/index.js',
      '@aurora/platform-audit/internal/repositories.js',
      '@aurora/platform-audit/repositories',
    ]) {
