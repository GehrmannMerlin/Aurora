...
/**
 * The platform-project-governance integration tests need the PLT-03 tables
 * (accounts, organizations, organization_members, project_members,
 * security_audit_events). This package may not depend on
...
const projectGovernanceMigrationsDir = fileURLToPath(new URL('../../migrations', import.meta.url));

/** This package's tables, in FK-safe drop order (children of projects first). */
const PROJECT_GOVERNANCE_TABLES_DROP_ORDER = [
  'client_keys',
...
/** Drop this package's tables + the PLT-03 tables (children first) + pgmigrations. */
export async function resetProjectGovernanceSchema(pool: Pool): Promise<void> {
  for (const table of PROJECT_GOVERNANCE_TABLES_DROP_ORDER) {
 ...
 * platform-project-governance migrations, in order.
 *
 ...
 * combined application order deterministic (identity, then project-governance).
 */
export async function runMigrationsUp(): Promise<void> {
  ...
  await runner({
    databaseUrl: testDatabaseUrl(),
    dir: projectGovernanceMigrationsDir,
    direction: 'up',
    migrationsTable: 'pgmigrations',
    count: Infinity,
    log: () => undefined,
...
/**
 * Run only this package's migration down (`count: 1` = the single project
 * governance migration). `checkOrder: false` for the same shared
 ...
 */
export async function runProjectGovernanceMigrationsDown(): Promise<void> {
  await runner({
    databaseUrl: testDatabaseUrl(),
    dir: projectGovernanceMigrationsDir,
    direction: 'down',
