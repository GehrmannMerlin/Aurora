...
function initialItemId(menu: 'organization' | 'project'): string | null {
  ...
  const current = props.projectOptions.find((project) => project.projectId === props.currentProjectId);
...
<template>
  <Teleport to="body">
    <nav v-if="openMenu !== null" class="au-scope-overlay" aria-label="作用域选择">
      <ul v-if="openMenu === 'organization'" id="organization-scope-menu" class="au-scope-menu" role="menu" aria-label="选择组织" :style="menuPosition">
        ...
        <li v-for="organization in organizations" :key="organization.organizationId" role="none">
          <button :id="itemId('organization', organization.organizationId)" class="au-scope-option" type="button" role="menuitem" :tabindex="itemTabIndex(itemId('organization', organization.organizationId))" :aria-current="organization.organizationId === currentOrganizationId ? 'true' : undefined" :aria-label="organization.organizationId === currentOrganizationId ? `${organization.name}（当前）` : organization.name" @focus="onItemFocus(itemId('organization', organization.organizationId))" @keydown="onItemKeydown" @click="emit('select-organization', organization.organizationId)">
      ...
      <ul v-if="openMenu === 'project'" id="project-scope-menu" class="au-scope-menu" role="menu" aria-label="选择项目" :style="menuPosition">
        ...
        <li v-for="project in projectOptions" :key="project.projectId" role="none">
          <button :id="itemId('project', project.projectId)" class="au-scope-option" type="button" role="menuitem" :tabindex="itemTabIndex(itemId('project', project.projectId))" :aria-current="project.projectId === currentProjectId ? 'true' : undefined" :aria-label="project.projectId === currentProjectId ? `${project.name}（当前）` : project.name" @focus="onItemFocus(itemId('project', project.projectId))" @keydown="onItemKeydown" @click="emit('select-project', project.projectId)">
