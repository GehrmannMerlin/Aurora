...
import { persistRequestEventSample } from '../src/request-sample-repository.js';
...
describe('persistRequestEventSample', () => {
  it('returns invalid_input when the input does not parse', async () => {
    ...
    const result = await persistRequestEventSample(pool, { projectId: 'p', eventEnvelope: {} });
    expect(result).toEqual({ status: 'invalid_input', code: 'invalid_envelope' });
    expect(insertCalls).toHaveLength(0);
  });

  it('maps a first insert to inserted with the sample id', async () => {
    const { client, queue, insertCalls, released } = makeClient();
    queue.push({ rows: [{ id: '7' }] });
    const pool = { connect: () => Promise.resolve(client) } as never;
    const result = await persistRequestEventSample(pool, validInput);
    ...
    expect(insert.params[1]).toBe('evt-request-repo-1');
    expect(insert.params[2]).toBe(1);
    expect(insert.params[4]).toContain('"method":"GET"');
    expect(insert.params[4]).not.toContain('protocolVersion');
  });
  ...
  it('does not mutate the input', async () => {
    ...
    await persistRequestEventSample(pool, input);
