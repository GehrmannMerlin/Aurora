...
async function runMigrations(direction: 'up' | 'down'): Promise<void> {
  ...
  const executed = await runner({
    ...
    count: Infinity,
    log: () => undefined,
    // The platform-identity/processing migrations share this pgmigrations table
    // but live in different directories. Disable node-pg-migrate's cross-directory
    // ordering assertion (it would otherwise reject this directory as "preceding"
    // the already-applied migrations). Same pattern as the integration test
    // helpers and the platform-audit runner.
    checkOrder: false,
  });
