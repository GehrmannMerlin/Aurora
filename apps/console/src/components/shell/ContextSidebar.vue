...
const props = withDefaults(defineProps<{ fill?: boolean; mobile?: boolean }>(), {
  fill: false,
  mobile: false,
});
...
function entriesFor(routeIds: readonly string[]): readonly RouteEntry[] {
  ...
  return routeIds
    .map((routeId) => ROUTE_BY_ID.get(routeId as RouteTargetId))
    .filter((entry): entry is RouteEntry => entry !== undefined && entry.menu);
}
...
<template>
  <nav
    v-if="isOrganization || isProject"
    class="au-context-sidebar"
    :class="{ 'au-context-sidebar--fill': props.fill, 'au-context-sidebar--mobile': props.mobile }"
...
.au-context-sidebar__group {
  margin-top: var(--space-5);
}
.au-context-sidebar__group h2 {
...
.au-context-sidebar :deep(.au-link) {
  display: flex;
...
.au-context-sidebar :deep(.au-link:hover),
.au-context-sidebar :deep(.au-link:focus-visible),
.au-context-sidebar :deep(.au-link--active) {
  color: var(--color-action-primary);
}
.au-context-sidebar--mobile {
  min-height: auto;
}
</style>
