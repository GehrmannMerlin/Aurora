...
describe('built platform-organization package entry', () => {
  it('loads the declared root entry', () => {
    const result = importFromPackage('@aurora/platform-organization');
    expect(result.status).toBe(0);
    expect(result.stderr).toBe('');
    expect(result.stdout).toContain('PLATFORM_ORGANIZATION_PACKAGE');
    expect(result.stdout).toContain('PLATFORM_ORGANIZATION_VERSION');
  });

  it('rejects private and unexported paths', () => {
    for (const specifier of [
      '@aurora/platform-organization/src/index.js',
      '@aurora/platform-organization/internal/repositories.js',
      '@aurora/platform-organization/repositories',
    ]) {
  ...
  it('declares the private data-layer package manifest', async () => {
    ...
    expect(manifest).toMatchObject({
      name: '@aurora/platform-organization',
      private: true,
      type: 'module',
      aurora: { layer: 'data' },
