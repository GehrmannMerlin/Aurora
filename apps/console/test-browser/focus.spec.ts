...
test('pagination clears the current-page issue selection and C5 endpoint selection supports Enter', async ({
  ...
  const issueList = page.waitForResponse(
    (response) =>
      response
        .url()
        .includes('/api/platform/v1/organizations/org_test_1/projects/prj_test_1/issues') &&
      response.request().method() === 'GET',
  );
  await page.goto(
    `${requiredServer().origin}/organizations/org_test_1/projects/prj_test_1/issues?status=open`,
  );
  ...
  await expect
    .poll(async () => (await issueListBody).data.issues.pagination.nextCursor)
    .toBe('cursor_2');
  ...
  await page.goto(
    `${requiredServer().origin}/organizations/org_test_1/projects/prj_test_1/requests`,
  );
