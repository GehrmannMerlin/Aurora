...
/**
 * The platform-audit integration tests need the PLT-03 tables
 * (organizations, organization_members, security_audit_events, accounts).
 * This package may not depend on `@aurora/platform-identity` (data → {protocol}
 * only), so the test helper runs the platform-identity migration directory via
 * node-pg-migrate's runner. This is test-only cross-directory migration
 * execution, not a package dependency.
 */
const identityMigrationsDir = fileURLToPath(
  new URL('../../../platform-identity/migrations', import.meta.url),
);

const auditMigrationsDir = fileURLToPath(new URL('../../migrations', import.meta.url));
...
export async function resetAuditSchema(pool: Pool): Promise<void> {
 ...
 * platform-audit migration (security_audit_events extension), in order.
 *
 ...
 * cross-directory ordering assertion would otherwise reject the second set as
 * "preceding" an already-applied migration. The schema is always reset first,
 * making the combined application order deterministic (identity, then audit).
 */
...
/**
 * Run only the platform-audit migration down (`count: 1` = this package's single
 * migration). `checkOrder: false` for the same shared `pgmigrations`-table
 * reason as `runMigrationsUp`. `count: Infinity` would trigger node-pg-migrate's
 * "definitions deleted" validation because the PLT-03 identity migration shares
 * the table from a sibling directory.
 */
export async function runAuditMigrationsDown(): Promise<void> {
  await runner({
    databaseUrl: testDatabaseUrl(),
    dir: auditMigrationsDir,
    direction: 'down',
...
export interface InsertTestAuditEventInput {
...
/**
 * Insert a security audit event directly via SQL. The platform-audit package is
 * READ-ONLY by design (B7), so the integration tests seed rows with a raw insert
 * instead of a repository write.
 */
export async function insertTestAuditEvent(
  pool: Pool,
  input: InsertTestAuditEventInput,
): Promise<string> {
  const result = await pool.query<{ event_id: string }>(
    `INSERT INTO security_audit_events
       (organization_id, actor_account_id, action, project_id, result, occurred_at, details)
