  ...
  <section class="au-surface" data-testid="members-view">
    ...
    <template v-else>
      <AppSection title="成员列表" description="电子邮箱以脱敏形式显示。">
        <template #actions>
          <div class="au-list-toolbar" role="toolbar" aria-label="成员列表操作">
            <AppButton variant="secondary" :disabled="loading" @click="void loadMembers()"
              >刷新成员</AppButton
            >
          </div>
        </template>
      ...
      <AppSection
        v-if="canManage"
        title="邀请成员"
        description="邀请仅在当前会话内列出，服务端会再次确认权限。"
      ...
      >
        ...
        <ul v-if="invitations.length > 0" class="au-invitation-list" data-testid="invitation-list">
          ...
          >
            ...
            <span class="au-invitation-expires"
              >过期时间（UTC） {{ formatUtc(invitation.expiresAt) }}</span
            >
            <AppTechnicalDetails summary="技术详情"
              >邀请状态键: {{ invitation.status }} 邀请 ID: {{ invitation.invitationId }} 过期时间
              (UTC): {{ invitation.expiresAt }}</AppTechnicalDetails
            >
