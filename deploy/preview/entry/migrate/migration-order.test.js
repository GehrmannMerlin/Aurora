...
test('monotonic email migrations remain deployable after the frozen baseline', () => {
  ...
  const result = analyzeMigrationOrder(
    [...sourceFiles, ...emailMigrations.map((name) => `${name}.ts`)],
    [...LEGACY_PRODUCTION_ORDER],
  );
  assert.equal(result.compatibility, 'approved-production-legacy');
  assert.equal(result.checkOrder, false);
  assert.deepEqual(result.pendingNames, emailMigrations);
});
