...
const resourcePolicyTarget = computed(
  () =>
    workspaceTargets.value.find((target) => target.routeId === 'platform.resource-policies') ??
    null,
);
...
function labelFor(routeId: (typeof items.value)[number]['routeId']): string {
  if (routeId !== 'account.notifications' || unreadBadge.value === null)
    return items.value.find((item) => item.routeId === routeId)?.label ?? '';
    ...
    <ul>
      <li v-for="item in items" :key="item.routeId">
        ...
        >
          ...
          <span
            v-if="item.routeId === 'account.notifications' && unreadBadge !== null"
            class="au-global-unread"
            aria-hidden="true"
            >{{ unreadBadge }}</span
          >
        </AppLink>
...
<style scoped>
.au-global-navigation {
  width: 100%;
}
.au-global-navigation ul {
  display: grid;
  gap: var(--space-2);
  margin: 0;
...
.au-global-navigation :deep(.au-link) {
  position: relative;
...
.au-global-navigation :deep(.au-link:hover),
.au-global-navigation :deep(.au-link:focus-visible),
.au-global-navigation :deep(.au-link--active) {
  background: rgb(255 255 255 / 12%);
  color: var(--color-rail-fg);
}
.au-global-navigation--expanded :deep(.au-link) {
...
.au-global-navigation--expanded :deep(.au-link:hover),
.au-global-navigation--expanded :deep(.au-link:focus-visible),
.au-global-navigation--expanded :deep(.au-link--active) {
...
.au-global-navigation--expanded .au-global-unread {
  position: static;
  margin-left: auto;
}
</style>
