...
async function runMigrations(direction: 'up' | 'down'): Promise<void> {
  ...
  const executed = await runner({
    ...
    count: Infinity,
    log: () => undefined,
    // The PLT-03 platform-identity and PLT-04 platform-organization migrations
    // share this pgmigrations table but live in sibling directories. Disable
    // node-pg-migrate's cross-directory ordering assertion (it would otherwise
    // reject this directory as "preceding" the already-applied upstream
    // migrations). Same pattern as the integration test helpers.
    checkOrder: false,
  });
