...
The canonical contract-testkit Navigation Context sample now contains the registered, server-authorized `platform.resource-policies` target so the conditional D2 UI entry is demonstrable without constructing a URL. This is not a production-discoverability claim: platform API, Worker, resource-policy and administration routes have their existing implementation boundaries, while the production Navigation Context producer remains blocked and leaves D2 discoverability as a residual gap.
