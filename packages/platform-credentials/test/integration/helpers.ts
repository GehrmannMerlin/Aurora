...
/**
 * The platform-credentials integration tests need the PLT-03 tables
 * (accounts, organizations, organization_members, security_audit_events) plus
 * the PLT-04 platform-organization migration (settings_version). This package
 * may not depend on `@aurora/platform-identity` or
 * `@aurora/platform-organization` (data → {protocol} only), so the test helper
 * runs both upstream migration directories via node-pg-migrate's runner. This
...
const organizationMigrationsDir = fileURLToPath(
  new URL('../../../platform-organization/migrations', import.meta.url),
);

const credentialsMigrationsDir = fileURLToPath(new URL('../../migrations', import.meta.url));

/** This package's tables, in FK-safe drop order. */
const CREDENTIALS_TABLES_DROP_ORDER = ['private_tokens'];
...
/** Drop this package's tables + the upstream tables (children first) + pgmigrations. */
export async function resetCredentialsSchema(pool: Pool): Promise<void> {
  for (const table of CREDENTIALS_TABLES_DROP_ORDER) {
 ...
 * platform-organization migration (settings version), then the
 * platform-credentials migration (private_tokens), in order.
 *
 * `checkOrder: false` on each runner call: each call targets the same shared
 * `pgmigrations` table but a different directory, so the runner's
 * cross-directory ordering assertion would otherwise reject the later sets as
 * "preceding" an already-applied migration. The schema is always reset first,
 * making the combined application order deterministic.
 */
...
/**
 * Run only this package's migration down (`count: 1` = the single private-tokens
 * migration). `checkOrder: false` for the same shared `pgmigrations`-table reason
 * as `runMigrationsUp`. `count: Infinity` would trigger node-pg-migrate's
 * "definitions deleted" validation because the PLT-03 upstream migrations share
 * the table from sibling directories.
 */
export async function runCredentialsMigrationsDown(): Promise<void> {
  await runner({
    databaseUrl: testDatabaseUrl(),
    dir: credentialsMigrationsDir,
    direction: 'down',
