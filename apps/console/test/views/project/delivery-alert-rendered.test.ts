...
describe('delivery and alerting rendered regressions', () => {
  it('selects a Source Map file by keyboard and renders only returned-file evidence', async () => {
    ...
    expect(screen.getByTestId('source-map-selected-file-technical').textContent).toContain(
      'sm_test_1',
    );
  });
  ...
  it('renders C12 evidence as read-only and does not expose raw instance identifiers', async () => {
    await router.push(
      '/organizations/org_test_1/projects/prj_test_1/alerts/instances/instance_test_1',
    );
    await router.isReady();
    ...
    expect(
      screen.getByTestId('alert-instance-status').querySelector('.mon-rule-name')?.textContent,
    ).not.toContain('rule_test_1');
