...
const visibleLines = computed(() => Array.from({ length: Math.max(1, props.lines) }, (_, index) => index));
</script>
...
<style scoped>
.au-skeleton { display: grid; gap: var(--space-2); }
.au-skeleton__label { color: var(--color-text-secondary); }
.au-skeleton__line {
  display: block;
...
.au-skeleton__line:last-child { width: 72%; }
@keyframes au-skeleton-pulse { to { opacity: 0.56; } }
@media (prefers-reduced-motion: reduce) {
  .au-skeleton__line { animation: none; }
}
</style>
