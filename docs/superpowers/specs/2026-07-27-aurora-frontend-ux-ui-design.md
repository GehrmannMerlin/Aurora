...
- 控制台视觉语言已经独立批准：浅色内容区、深石墨顶栏、纯色琥珀橙侧栏、深色前景、中高信息密度且禁止渐变；精确实现见[控制台视觉语言设计](./2026-07-30-aurora-console-visual-language-design.md)，主题代码、组件与浏览器证据仍是后续实现门禁；
...
- 正式视觉方向为浅色内容区、`#111827` 深石墨顶栏、`#D47A16` 纯色琥珀橙侧栏与深色前景；当前路由使用 `#FFF4DC` 浅色选中行和 `#1D4ED8` 蓝色标识；侧栏及正式表面禁止渐变、纹理、光晕和玻璃拟态；
- 状态不能只靠颜色表达，必须同时使用文本和必要图标；成功、警告、错误、禁用和数据不完整保持一致语义，导航琥珀橙不得解释为告警色；
- 顶栏和侧栏在窄屏折叠为可关闭导航抽屉，层级和入口不变；主从工作区改为列表后详情顺序，不隐藏危险说明、完整性或权限状态；
...
- 系统无衬线字体、中高信息密度、4px 基础间距序列和 6px 常规圆角已经批准；断点、动画参数和真实组件校正由前端实现按[控制台视觉语言设计](./2026-07-30-aurora-console-visual-language-design.md)收口，不能另建页面私有主题。
...
| VISUAL-001 | 控制台视觉语言 | 浅色工作区＋深石墨顶栏＋纯色琥珀橙侧栏；深色导航前景、中高信息密度、蓝色主操作和选中标识；禁止渐变、纹理与页面私有主题 | 用户在 C2 完成态视觉方案中选择第一张橙色侧栏方向，并授权同方向低风险视觉细节由 Agent 直接收口 | `approved`；不授权主题、组件或页面实现；业务/权限/安全/数据/公共契约仍遵守原门禁 | A1—D2、NAV-A、公共壳层与 Aurora UI |
...
| 视觉方向与实现状态 | 通过；纯色琥珀橙侧栏、浅色内容区和禁止渐变已经批准，主题代码、组件和浏览器证据继续标为不存在 |
