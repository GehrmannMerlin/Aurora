  ...
  return {
    ...
    async claimOutboxRows(pool: Pool | PoolClient, input) {
      const result = await pool.query<OutboxRowShape>(
        `WITH exhausted AS (
           UPDATE outbox
           SET status = 'dead_lettered', payload = '{}'::jsonb, claim_id = NULL,
           ...
           WHERE attempt_count >= $4
             AND (status IN ('pending', 'failed') OR
                  (status = 'processing' AND
                   updated_at <= $2::timestamptz - ($3::bigint * interval '1 millisecond')))
         ), claimable AS (
           SELECT outbox_id FROM outbox
           WHERE attempt_count < $4
             AND ((status IN ('pending', 'failed') AND available_at <= $2::timestamptz)
         ...
         SET status = 'processing', claim_id = gen_random_uuid(),
             attempt_count = target.attempt_count + 1, updated_at = $2::timestamptz
         FROM claimable
        ...
        [input.limit, input.now.toISOString(), input.processingTimeoutMs, input.maxAttempts],
      );
